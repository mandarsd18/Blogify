...
const AllBlogs = () => {
  ...
  const getData = async () => {
    const res = await axios.get("https://blogify-ioe8.onrender.com/blogs/allBlogs");
  ...
  return (
    <>
    <h1 className="text-center text-xl font-bold text-white sm:text-2xl md:text-4xl mt-4 mb-6">
        All Blogs
      </h1>
      <div className="w-[90%] sm:w-[80%] mx-auto flex flex-wrap items-center justify-center gap-5">
        {data.map((item) => (
          <div key={item._id} className="flex flex-col text-white  w-[260px] md:w-[280px] border border-white p-2 rounded-md">
